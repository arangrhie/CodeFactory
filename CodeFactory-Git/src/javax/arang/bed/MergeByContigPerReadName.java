...
public class MergeByContigPerReadName extends IOwrapper {
	...
	public void hooker(FileReader fr, FileMaker fm) {
		...
		while (fr.hasMoreLines()) {
			...
			if (!tokens[Bed.NOTE].equals(prevReadName) || !tokens[Bed.CHROM].equals(prevChr) || isDiscordant) {
				if (isFirst) {
					isFirst = false;
					fm.writeLine("#CHR\tSTART\tEND\tREADNAME\tRANGE\tCOVERED_RANGE\tNUM_READS\tAVG_DEPTH\tCHR_LEN");
				} else {
					// write down
	...
	public void printHelp() {
		...
		System.out.println("\t<in.bed>: sorted bed file. CHR\tSTART\tEND\tREADNAME\tLEN\tNUM_READS\tAVG_DEPTH\tCHR_LEN");
		System.out.println("\t<out.bed>: CHR\tSTART\tEND\tREADNAME\tRANGE\tCOVERED_RANGE\tNUM_READS\tAVG_DEPTH\tCHR_LEN");
